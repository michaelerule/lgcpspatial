...
"""
The goal here is to design a single monolithic function that encapsulates 
...
def tril(M,k=0):
    ...
    if len(M.shape)==1:
        ...
        # Jax style
        # return np.zeros((N,N),dtype=rtype).at[np.tril_indices(N)].set(M)
        # Numpy style
...
tdivl = lambda A,B: jax.scipy.linalg.solve_triangular(A,B,lower=True,dtype=rtype)
tdivu = lambda A,B: jax.scipy.linalg.solve_triangular(A,B,lower=False,dtype=rtype)
...
class fourier_lowrank:
    ...
    discarded, and Q is a low-rank matrix.
    '''
    def __init__(self,n,y,lλf,lλb,L,σ0,p,
        ...
        Rlt    = (R*(R+1))//2                          # Elements in lower-triangular RxR
        ...
        self.cached = (n,y,bg,L,R,Rlt,use2d,F,K,Λ,h2e,M)

    def covariance_diagonal(self,Q):
        n,y,bg,L,R,Rlt,use2d,F,K,Λ,h2e,M = self.cached
        return sum((F.T@tril(Q))**2,1)
    
    def μλ(self,μh,Q):
        n,y,bg,L,R,Rlt,use2d,F,K,Λ,h2e,M = self.cached
        μ = F.T@μh
        v = self.covariance_diagonal(Q)
        return μ, sexp(μ + v/2 + bg)

    def loss(self,μh,Q):
        n,y,bg,L,R,Rlt,use2d,F,K,Λ,h2e,M = self.cached
        μ,λ=  self.μλ(μh,Q)
        l1 =  n@(λ-y*μ)         #  n'(λ-y∘μ)
        l2 =  sum(μh**2*Λ     , dtype=rtype)/2  #  ½μ'Λ₀μ
        l3 = -sum(log(Λ)      , dtype=rtype)/2  #  ½ln|Σ₀|
        l4 = -sum(log(diag(Q)), dtype=rtype)    # -½ln|Σ|
        l5 =  sum((Q@Q.T)*Λ   , dtype=rtype)/2  #  ½tr[Λ₀Σ] = ½tr[Λ₀QQ'] = ½tr[Λ₀QQ']
        return l1+l2+l3+l4+l5

    def likelihood(self,μh,Q):
        n,y,bg,L,R,Rlt,use2d,F,K,Λ,h2e,M = self.cached
        μ,λ  =  self.μλ(μh,Q)
        nyλ  =  n@(y*μ - λ)
        nλ   =  n*λ
    ...
    def JHvMQ(self,μh,Q):
        ...
        def Hv(A):
            ...
            ΛA  = Λ [:,None] * A
            PAP = Pt @ A.T @ Pt
            return tril( F@(dFA + rFQ) + ΛA + PAP )
        
        Hv = op(Rlt,Hv)
        
        # J = tril_elements(JQ(μ,Q))
        nλ     = n*exp(μ+bg+sum(FQ**2,1)/2)
        FdλFtQ = F@(nλ[:,None]*FQ)
        FΛ0FtQ = Λ[:,None]*Q
        J      = tril(FΛ0FtQ + FdλFtQ - Pt)
    
        M = op(Rlt,lambda M:tril(K[:,None]*tril(M)))
    
        return J,Hv,M
    
    def mean_update(self,μh,Q):
        J,Hv,M = self.JHvMμ(μh,Q)
        return - minres(Hv,J,tol=self.mintol,M=M)[0]
    
    def variance_update(self,μh,Q):
        J,Hv,M = self.JHvMQ(μh,Q)
        return - minres(Hv,J,tol=self.mintol,M=M)[0]
